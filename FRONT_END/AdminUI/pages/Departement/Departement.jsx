...
import { Table } from "../../../../FRONTEND/Components/Details/DetailsDepartement/Table";
import { Modal } from "../../../../FRONTEND/Components/Details/DetailsDepartement/Modal";
...
function Departement() {
  ...
  const handleDeleteRow = (targetIndex) => {
    ...
    fetch(
      `http://localhost:8080/departements/deleteDepartement/${departementToDelete.id_département}`,
      {
        method: "DELETE",
      }
    )
  ...
  const handleSubmit = (newRow) => {
    ...
    fetch(url, {
      ...
      .then((response) => {
        if (!response.ok)
          throw new Error("Erreur lors de la soumission du département");
      ...
      .then((data) => {
        if (rowToEdit === null) {
          ...
          setRows(
            rows.map((currRow, idx) => (idx !== rowToEdit ? currRow : data))
