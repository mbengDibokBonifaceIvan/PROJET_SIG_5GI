...
import "./Region.css";
import { Table } from "../../../../FRONTEND/Components/Details/DetailsRegion/Table";
import { Modal } from "../../../../FRONTEND/Components/Details/DetailsRegion/Modal";
  ...
  const handleSubmit = async (newRegion) => {
    try {
      if (regionToEdit === null) {
        ...
        const response = await fetch(
          "http://localhost:8080/regions/addRegion",
          {
            method: "POST",
        ...
        if (!response.ok)
          throw new Error("Erreur lors de la modification de la région");

        const updatedRegion = await response.json();
        setRegions(
          regions.map((region, idx) =>
            idx === regionToEdit ? updatedRegion : region
          )
        );
      }
    ...
    <div className="Region">
      ...
      {modalOpen && (
        <Modal
          ...
          defaultValue={
            regionToEdit !== null ? regions[regionToEdit] : { nom_région: "" }
          }
        />
      )}
    </div>
