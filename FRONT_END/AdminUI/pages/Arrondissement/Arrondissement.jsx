...
import { Table } from "../../../../FRONTEND/Components/Details/DetailsArrondissement/Table";
import { Modal } from "../../../../FRONTEND/Components/Details/DetailsArrondissement/Modal";
...
function Arrondissement() {
  ...
  const fetchArrondissements = async () => {
    try {
      ...
      if (!response.ok)
        throw new Error("Erreur lors de la récupération des arrondissements");
  ...
  const handleSubmit = async (newRow) => {
    if (rowToEdit === null) {
      ...
      try {
        const response = await fetch(
          "http://localhost:8080/arrondissements/addArrondissement",
          {
            method: "POST",
            headers: {
        ...
        if (!response.ok)
          throw new Error("Erreur lors de l'ajout de l'arrondissement");
      ...
      try {
        ...
        if (!response.ok)
          throw new Error("Erreur lors de la mise à jour de l'arrondissement");
