...
function CentreVote() {
  ...
  useEffect(() => {
    ...
    const fetchArrondissements = async () => {
      try {
        const response = await fetch("http://localhost:8080/arrondissements/all");
        if (!response.ok) throw new Error("Erreur de récupération des arrondissements");
        const data = await response.json();
        setArrondissements(data);
      } catch (error) {
        console.error("Erreur :", error);
  ...
  const handleDeleteRow = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/centres-de-vote/deleteCentreDeVote/${id}`, {
        method: "DELETE",
      });
  ...
  const handleSubmit = async (newRow) => {
    if (rowToEdit === null) {
      ...
      try {
        const response = await fetch(`http://localhost:8080/centres-de-vote/editCentreDeVote/${rowToUpdate.id_centre_vote}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
